...
export const ColorPanel: React.FC<ColorPanelProps> = ({ config, updateConfig, onGeneratePalette, onGenerateVibe, isGenerating, activeColorIndex, setActiveColorIndex }) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Quick controls (always on top of the panel) */}
      <div>
        <h3 className="text-xs text-gray-300 uppercase font-semibold mb-2">Quick Color</h3>
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="flex-1 bg-gray-900/70 border border-gray-600 font-mono text-white placeholder-gray-400 text-sm rounded-md px-3 py-2 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none"
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={handleAddColor} className="w-8 h-8 rounded-full bg-gray-700 hover:bg-gray-600 text-gray-200 flex items-center justify-center" aria-label="Add new color">
            <PlusIcon className="w-5 h-5" />
          </button>
          <button onClick={shuffleColors} className="w-8 h-8 rounded-full bg-gray-700 hover:bg-gray-600 text-gray-200 flex items-center justify-center" aria-label="Shuffle colors">
      ...
      <details>
        <summary className="cursor-pointer text-sm text-gray-300">Advanced: Generate with AI</summary>
        <div className="mt-2 flex flex-col gap-2">
          <input
            ...
            className="w-full bg-gray-900/70 border border-gray-600 text-white placeholder-gray-400 text-sm rounded-md px-3 py-2"
          />
          <div className="flex gap-2">
            <button 
              onClick={() => handleGenerateClick(onGeneratePalette)}
              disabled={isGenerating}
              className="flex-1 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 flex items-center justify-center gap-2 disabled:bg-blue-800"
            >
            ...
            <button 
              ...
              disabled={isGenerating}
              className="flex-1 px-4 py-2 text-sm font-medium text-white bg-purple-600 rounded-md hover:bg-purple-700 flex items-center justify-center gap-2 disabled:bg-purple-800"
            >
              <SparklesIcon className={`w-4 h-4 ${isGenerating ? 'animate-spin' : ''}`} />
      ...
      <details>
        <summary className="cursor-pointer text-sm text-gray-300">Advanced: Presets</summary>
        <div className="mt-2 flex flex-wrap gap-2">
            {PRESETS.map(preset => (
                <button
                    key={preset.name}
                    onClick={() => handlePresetClick(preset.config)}
                    className="px-3 py-1 text-sm bg-gray-700 text-gray-200 rounded-full hover:bg-gray-600 hover:text-white"
                >
                    {preset.name}
                </button>
            ))}
