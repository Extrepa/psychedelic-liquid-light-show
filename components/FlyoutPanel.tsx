...
export const FlyoutPanel: React.FC<FlyoutPanelProps> = ({ isOpen, onClose, children, title }) => {
  ...
  return (
    <div 
        className={`fixed top-4 left-4 bottom-24 z-20 w-full max-w-xs sm:w-80 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-[110%]'}`}
        role="dialog"
        aria-modal="true"
        aria-hidden={!isOpen}
    >
